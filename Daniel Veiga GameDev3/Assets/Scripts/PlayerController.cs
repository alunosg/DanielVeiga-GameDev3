...
public class PlayerController : MonoBehaviour
{
    ...
    public void Jump(InputAction.CallbackContext context)
    {
        if (context.started)
        {
            ...
            Vector3 p5 = new(col.bounds.center.x, col.bounds.min.y, col.bounds.center.z);
            if (Physics.Raycast(p1, Vector3.down, col.bounds.size.y * 1.01f, floorlayer) ||
                Physics.Raycast(p2, Vector3.down, col.bounds.size.y * 1.01f, floorlayer) ||
                Physics.Raycast(p3, Vector3.down, col.bounds.size.y * 1.01f, floorlayer) ||
                Physics.Raycast(p4, Vector3.down, col.bounds.size.y * 1.01f, floorlayer) ||
                Physics.Raycast(p5, Vector3.down, col.bounds.size.y * 1.01f, floorlayer))
            {
